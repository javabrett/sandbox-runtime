...
function generateSandboxProfile({
  ...
  const profile: string[] = [
    ...
    '; security.mac.sandbox.sentinel: conventionally readable only when NOT sandboxed.',
    '; The security CLI (and similar tools) read this to detect sandbox state and',
    '; bail out early with EPERM before attempting Keychain operations. Allowing',
    "; this read lets those tools proceed; srt's seatbelt rules remain the actual",
    ...
    '; com.apple.SecurityServer: legacy securityd, handles Keychain IPC on macOS <= 12',
    '(allow mach-lookup (global-name "com.apple.SecurityServer"))',
    '; com.apple.secd: Security Entry Daemon, primary Keychain IPC on macOS 13+ (Ventura+)',
    '; Required for credential read/write/delete via the apple-tool: partition on modern macOS.',
    '; Without this, Keychain writes (e.g. OAuth token refresh) silently fail on Ventura and later',
    '; even though SecurityServer is allowed - secd is the actual handler for those operations.',
